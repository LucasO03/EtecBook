...
public class AccountController : ControllerBase
{
    ...
    public async Task<IActionResult> Authenticate([FromBody] LoginDto login)
    {
        ...
        if (user == null)
            return NotFound(new { Message = "Usuário e/ou Senha Inválidas" });
    ...
    public async Task<IActionResult> Register([FromBody] RegisterDto register)
    {
        ...
        if (await _context.Users.AnyAsync(u => u.Email.Equals(register.Email)))
            return BadRequest(new { Message = "Email já cadastrado. Tente recuperar sua senha ou entre me contato com o Administrador" });
